...
class DrpEnv(gym.Env):
	def __init__(self,
		...
		self.colli_distan_value = float(speed)
		self.r_flag = 0
		...
		try:
			...
			class LaReArgs:
				def __init__(self, env_instance):
					...
					self.device = env_instance.device 				# device for LARE model
			...
			self.lare_decompose.device = self.device  # サブプロセスのdeviceに合わせて上書き
			self.lare_decompose.reward_model.device = self.device  # Factor_Reward_Model.deviceも上書き
	...
	def step(self, joint_action):
		...
		# 💥 修正: 衝突ペアのリストを取得
		colliding_pairs = self.ee_env.get_collision_agents(self.obs_prepare, self.colli_distan_value)
