...
def make_train_step(model, loss_fn, optimizer, n_agent, device, env = None, reg=False, alpha=None):
    # Builds function that performs a step in the train loop
    if reg: assert alpha is not None

    def _log(messege):
        if env is not None and hasattr(env, '_log'):
            env._log(messege)
        else:
            print(messege)

    def train_step(states, actions, episode_return, episode_length, next_states=None):
        #states: b,na,t,d
        # actions: b,na,t
        # episode_return: b
        # episode_length: b
        ...
        if torch.isnan(states).any():
            print(f"  ⚠️ [NaN] States contain NaN! Count: {torch.isnan(states).sum().item()}")
        if torch.isinf(states).any():
            print(f"  ⚠️ [Inf] States contain Inf! Count: {torch.isinf(states).sum().item()}")
        if torch.isnan(episode_return).any():
            print(f"  ⚠️ [NaN] Episode return contains NaN!")
            print(f"    Values: {episode_return[torch.isnan(episode_return)]}")

        # Makes predictions: pred_return: [batch_size, n_agent, time length]
        try:
            if next_states is not None:
                pred_rewards = model(states, actions, episode_length, next_states).squeeze()
            else:
                pred_rewards = model(states, actions, episode_length).squeeze()
            
            # print(f"  - Pred rewards shape: {pred_rewards.shape}")
            
            # 🔍 予測報酬のチェック
            if torch.isnan(pred_rewards).any():
                print(f"  ⚠️ [NaN] Predicted rewards contain NaN! Count: {torch.isnan(pred_rewards).sum().item()}")
                print(f"    Sample values: {pred_rewards.flatten()[:10]}")
            if torch.isinf(pred_rewards).any():
                print(f"  ⚠️ [Inf] Predicted rewards contain Inf! Count: {torch.isinf(pred_rewards).sum().item()}")
        ...
        # 🔍 マスク適用後の報酬
        masked_pred_rewards = pred_rewards * main_mask
        summed_pred_rewards = masked_pred_rewards.sum(dim=[1, 2])

        # 🔍 NaN/Infチェック（マスク適用後）
        if torch.isnan(summed_pred_rewards).any():
            print(f"  ⚠️ [NaN] Summed predicted rewards contain NaN!")
            nan_indices = torch.where(torch.isnan(summed_pred_rewards))[0]
            print(f"    Batch indices with NaN: {nan_indices}")
            for idx in nan_indices[:3]:  # 最初の3つを表示
                print(f"    Batch {idx}:")
                print(f"      - pred_rewards: {pred_rewards[idx]}")
                print(f"      - mask: {main_mask[idx]}")
                print(f"      - episode_length: {episode_length[idx]}")
        
        # 元の実装: 総報酬で直接比較
        try:
            loss = loss_fn(episode_return, summed_pred_rewards)
            
            _log(f"  - Loss value: {loss.item()}")
            
            # 🔍 損失のチェック
            if torch.isnan(loss):
                print(f"  ⚠️ [NaN] Loss is NaN!")
                print(f"    - episode_return stats:")
                print(f"      mean: {episode_return.mean().item()}")
                print(f"      std: {episode_return.std().item()}")
                print(f"      min: {episode_return.min().item()}")
                print(f"      max: {episode_return.max().item()}")
                print(f"    - summed_pred_rewards stats:")
                print(f"      mean: {summed_pred_rewards.mean().item()}")
                print(f"      std: {summed_pred_rewards.std().item()}")
                print(f"      min: {summed_pred_rewards.min().item()}")
                print(f"      max: {summed_pred_rewards.max().item()}")
        ...
        if reg:
            ...
            loss += alpha*reg_loss
            print(f"  - Total loss (with reg): {loss.item()}")
...
def make_arel_train_step(model, loss_fn, optimizer, n_agent, device, reg=False, alpha=None):
    # Builds function that performs a step in the train loop
    def train_step(states, actions, episode_return, episode_length, next_states=None):
        # Sets model to TRAIN mode
        model.train()
        # Makes predictions
        yhat, y_time_hat = model(next_states)
        # Computes loss
        loss = loss_fn(episode_return, yhat)
...
def make_rrd_train_step(args, model, loss_fn, optimizer, n_agent, device, reg=False, alpha=None):
    # Builds function that performs a step in the train loop
    if reg: assert alpha is not None
    def train_step(states, actions, episode_return, episode_length, next_states=None):
        model.train()
        ...
        # Makes predictions: pred_return: [batch_size, n_agent, time length]
        if next_states is not None:
            pred_rewards, sampled_rewards_var = model(states, actions, episode_length, next_states,only_inference=False)
            pred_rewards = pred_rewards.squeeze()
        else:
            pred_rewards, sampled_rewards_var = model(states, actions, episode_length,only_inference=False)
            pred_rewards = pred_rewards.squeeze()
        # Computes loss
        ...
        if args.rrd_unbiased:
            loss -= sampled_rewards_var
...
def make_IB_train_step(args, model, loss_fn, optimizer, n_agent, device, reg=False, alpha=None):
    # Builds function that performs a step in the train loop
    if reg: assert alpha is not None
    def train_step(states, actions, episode_return, episode_length, next_states=None):
        # Sets model to TRAIN mode
        ...
        # Makes predictions: pred_return: [batch_size, n_agent, time length]
        input_episode_return = episode_return.unsqueeze(-1).unsqueeze(-1).repeat(1,n,t).unsqueeze(-1).to(states.device)/(n*t)
        if next_states is not None:
            pred_rewards, kl_loss = model(states, actions, input_episode_return, next_states, return_kl=True)
            pred_rewards = pred_rewards.squeeze()
        else:
            pred_rewards, kl_loss = model(states, actions, input_episode_return, return_kl=True)
            pred_rewards = pred_rewards.squeeze()
        # Computes loss
