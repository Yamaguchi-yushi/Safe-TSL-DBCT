...
class DRP_prompt(Base_prompt):
    def __init__(self, map_name, factor_decomp=False, n_agents=None, collision_dis=None, speed=None) -> None:
        ...
        # 衝突判定距離の設定
        self.collision_dis = collision_dis if collision_dis is not None else 5.0
        
        # 移動速度の設定
        self.speed = speed if speed is not None else 5.0
        
        # 観測次元の計算: agent_location + goal_location + other_agents_relative + collision_distance + collsion_info + wait_count + node_info
...
Please suggest appropriate next actions for each agent based on this task.\n\n"
        ...
        base_index_for_node_coords = self.n_nodes * 2 + self.n_nodes + (self.n_agents - 1) * self.n_nodes * 2 + 1 + 2 + 1
...
- The collision information: a 2-dimensional vector where the first element indicates whether the agent is involved in a collision (1 if yes, 0 if no), and the second element indicates whether there are other agents involved in any collision (1 if yes, 0 if no). \
- The wait count: a scalar value indicating the number of consecutive time steps the agent has remained in the same position. This value resets to 0 when the agent moves. Importantly, this counter does not increment if the agent is waiting at its assigned goal node. \
...
To access coordinates for node i: x_coordinate = observation[base_index + i*2], y_coordinate = observation[base_index + i*2 + 1], where base_index = {base_index_for_node_coords}. \n\
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='DRP Prompt Generator')
    parser.add_argument('--map_name', '-m', type=str, default='map_8x5',
                        help='Map name (map_3x3, map_5x5, map_8x5, aoba01)')
    parser.add_argument('--factor_decomp', '-f', action='store_true',
                        help='Enable factor decomposition')
    parser.add_argument('--n_agents', '-n', type=int,
                        help='Number of agents (default: map-specific)')
    parser.add_argument('--collision_dis', '-c', type=float,
                        help='Collision detection distance (default: 0.1)')
    parser.add_argument('--speed', '-s', type=float,
                        help='Agent movement speed (default: 5.0)')
    ...
    try:
        # プロンプト生成
        prompt_obj = DRP_prompt(
            args.map_name, 
            factor_decomp=args.factor_decomp,
            n_agents=args.n_agents,
            collision_dis=args.collision_dis,
            speed=args.speed
        )
        
        if args.debug:
            print(f"Map: {args.map_name}")
            print(f"Nodes: {prompt_obj.n_nodes}")
            print(f"Agents: {prompt_obj.n_agents}")
            print(f"Edges: {prompt_obj.n_edges}")
            print(f"Collision distance: {prompt_obj.collision_dis}")
            print(f"Speed: {prompt_obj.speed}")
            print(f"Observation dim: {prompt_obj.obs_dim}")
            print("="*50)
        
        # プロンプトメッセージの生成
        messages = prompt_obj.get_message()
